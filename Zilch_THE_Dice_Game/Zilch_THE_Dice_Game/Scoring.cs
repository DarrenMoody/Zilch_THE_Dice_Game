...
namespace Zilch_Dice_Game {
	...
	public class Scoring { 
		...
		// Parameters: none
		// Returns: none
		private void resetPoints() // DARREN - ***********************
		{
			for (int i = 0; i < 6; i++) // resets 2D array
			{
				numClicks[i] = 0;
				diceCounter[i] = 0;
				diceValues[i] = 0;
		...
		public void scoringTraverse() { // DARREN - ***********************
            ...
            */
			...
			for (int i = 0; i < 6; i++) {
				if (zilch) {
					/*btnPoints[i].Enabled = true;
                    btnPoints[i].Visible = true;
                    btnPoints[i].Text = "ZILCH!";*/
				...
				if (printed[0] == false) // If this is false...
				{
					if (sixKind) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Six of a kind (" + points6Kind + ")"; // points6Kind = same as points3Pair (1500). 
						scorePos[i] = points6Kind;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == value6Kind) {
				...
				if (printed[1] == false) {
					if (sixOnes) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Two sets of three 1s (" + pointsSixOnes + ")";
				...
				if (printed[2] == false) {
					if (fullHouse) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Full House (" + pointsFHouse + ")";
						scorePos[i] = pointsFHouse;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == fhVal1) {
								if (fhTripleCounter < 3) {
									btnPointsCheck[j, i] = j + 1;
								}
								fhTripleCounter++;
							} else if (diceValues[j] == fhVal2) {
								if (fhPairCounter < 2) {
									btnPointsCheck[j, i] = j + 1;
								}
								fhPairCounter++;
				...
				if (printed[3] == false) {
					if (fiveKind) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Five of a kind (" + points5Kind + ")";
						scorePos[i] = points5Kind;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == value5Kind) {
								btnPointsCheck[j, i] = j + 1;
							}
						}
						goto printing;
				...
				if (printed[4] == false) {
					if (fiveOnes) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Five of a kind (" + points5Ones + ")";
				...
				if (printed[5] == false) {
					if (fourKind) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Four of a kind (" + points4Kind + ")";
						scorePos[i] = points4Kind;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == value4Kind) {
								btnPointsCheck[j, i] = j + 1;
							}
						}
						goto printing;
				...
				if (printed[6] == false) {
					if (fourOnes) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Four ones (" + points4Ones + ")";
				...
				if (printed[7] == false) {
					if (threeKind) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Three of a kind (" + points3Kind + ")";
						scorePos[i] = points3Kind;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == value3Kind) {
								btnPointsCheck[j, i] = j + 1;
							}
						}
						goto printing;
				...
				if (printed[8] == false) {
					if (second3Kind) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Three of a kind (" + points23Kind + ")";
						scorePos[i] = points23Kind;
						for (int j = 0; j < 6; j++) // Loop to assign positions of dice for one of btnPoints1-5
						{
							if (diceValues[j] == value23Kind) {
								btnPointsCheck[j, i] = j + 1;
							}
						}
						goto printing;
				...
				if (printed[9] == false) {
					if (threeOnes) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Three Ones (" + points3Ones + ")";
				...
				if (printed[10] == false) {
					if (twoOnes) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Two Ones (" + points2Ones + ")";
				...
				if (printed[11] == false) {
					if (twoFives) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Two Fives (" + points2Fives + ")";
				...
				if (printed[12] == false) {
					if (straight) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Straight (" + pointsStraight + ")";
				...
				if (printed[13] == false) {
					if (oneOne) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "One 1 (" + points1One + ")";
				...
				if (printed[14] == false) {
					if (oneFive) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "One Five (" + points1Five + ")";
				...
				if (printed[15] == false) {
					if (threePair) {
						...
						btnPoints[i].Enabled = true;
						btnPoints[i].Visible = true;
						btnPoints[i].Text = "Three Pairs (" + points3Pair + ")";
